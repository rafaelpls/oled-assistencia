...
export class SupabasePhotoStorage implements PhotoStorage {
  ...
  constructor() {
    ...
    if (process.env.PHOTO_STORAGE === 'supabase' && (!this.url || !this.token))
      throw new Error(
  ...
  private async request(method: 'PUT' | 'GET' | 'DELETE', key: string, body?: Buffer) {
    ...
    if (!res.ok && !(method === 'DELETE' && res.status === 404))
      throw new Error(`Supabase Storage retornou status HTTP ${res.status}.`);
    return res;
  }
